...
def _row_to_subscription(row: sqlite3.Row) -> Dict[str, Any]:
    return {
        ...
        "push_targets": row["push_targets"],
        "feishu_webhook": row["feishu_webhook"],
        "wechat_work_webhook": row["wechat_work_webhook"],
    ...
    with db() as conn:
        cur = conn.execute(
            ...
            (
                ...
                data.get("push_targets", "primary"),
                data.get("feishu_webhook", ""),
                data.get("wechat_work_webhook", ""),
